...
- `exclamation_marks`: Each additional ! multiplies odds of spam by `r format(round(odds_ratios[2], 2), scientific = TRUE)`
- `contains_free`: Having "free" multiplies odds by `r format(round(odds_ratios[3], 2), scientific = TRUE)`
- `length`: Each additional character multiplies odds by `r format(round(odds_ratios[4], 4), scientific = FALSE)` (shorter = more likely spam)
