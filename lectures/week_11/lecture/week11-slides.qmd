...
**Remember Week 6? Predictive features from spatial data**
...
:::{.fragment}
**Now, predictive features from temporal data:**
...
- Never use spatial lags in prediction
  - Creates circularity

- Temporal lags are good!
  - Time is directional
  - Data from the past predicts the future
  - (Future does not predict the past)
...
## expand_grid() to the Rescue

```{r eval=FALSE, echo=TRUE}
# Create every possible station-hour combination
study.panel <- expand_grid(
...
**Week 7: Spatial cross-validation (Leave-One-Group-Out)**
